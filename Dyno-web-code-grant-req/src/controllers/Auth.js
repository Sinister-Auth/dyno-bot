	...
	async return(bot, req, res) {
		...
		if (config.isPremium || config.staging) {
			if (!req.query || !req.query.guild_id) {
				...
				return res.render('message', { message: `You don't have permissions to invite this bot.` });
			}

			if (!config.staging) {
				try {
					const guild = await models.Server.findOne({ _id: req.query.guild_id });
				...
				try {
					...
					if (!globalConfig || !globalConfig.authorizedGuilds || !globalConfig.authorizedGuilds.find(id => id === req.query.guild_id)) {
						res.locals.stylesheets.push('legal');
                                                return res.render('message', { message: `You don't have permissions to invite this bot.` });
					}
				} catch (err) {
