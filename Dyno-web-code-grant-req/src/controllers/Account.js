...
class Account extends Controller {
	...
	async ingest(bot, req, res) {
		...
		try {
				...
				case braintree.WebhookNotification.Kind.SubscriptionExpired:
					...
					if (subscription.premiumUserId && !dynoPremiumSubscriptions.length) {
						try {
							...
							logger.error(err, 'AccountIngest');
						}
					}
			...
			logger.error(err, 'AccoutIngest', notification);
			...
			if (premiumUser.subscriptions.findIndex(i => i == req.params.id) >= 0) {
				...
				if (subscription.cancelled) {
					...
					if (!cancelResponse.success) {
						logger.error('Braintree returned an error when cancelling', 'AccountCancel', cancelResponse);
	...
	async data(bot, req, res) {
		...
		let braintreeGuilds = activatedGuilds.filter((g) => g.subscriptionType === 'braintree');
		let patreonGuilds = activatedGuilds.filter((g) => g.subscriptionType !== 'braintree');
	...
	async activate(bot, req, res) {
		...
		let guildsQuery = await models.Server
			.aggregate([{ $match: { premiumUserId: req.session.user.id, subscriptionType: 'braintree' } }, { $count: 'activated_servers' }]);
		...
		try {
			...
			await models.Server.updateOne({ _id: serverId }, { $set: { isPremium: true, premiumUserId: req.session.user.id, premiumSince: new Date().getTime(), subscriptionType: 'braintree' } });
