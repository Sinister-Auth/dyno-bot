...
class Dyno {
	...
	constructor() {
		...
		parentPort.once('message', message => {
			if (!message.port) {
				throw new Error('Failed to receive Message Port.');
			}
	...
	async setup(options, rootContext) {
		...
		this.readyTimeout = setTimeout(() => {
			try {
				...
				logger.error(`Message Port Error:`, err);
			}
		}, 90000);
	}
	...
	updateGlobal(change) {
		...
		if (this._globalConfig.playingStatus) {
			if (this._globalConfig.playingStatus[this.config.client.id] && this._globalConfig.playingStatus[this.config.client.id] !== this.playingStatus) {
				this.updateStatus(this._globalConfig.playingStatus[this.config.client.id]);
			} else if (this._globalConfig.playingStatus.default && this._globalConfig.playingStatus.default !== this.playingStatus) {
				this.updateStatus(this._globalConfig.playingStatus.default);
			}
		}
	}
