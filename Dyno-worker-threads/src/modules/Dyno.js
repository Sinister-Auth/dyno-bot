		...
		this._blocked = blocked(ms => {
			if (ms > 10000) {
				this.statsd.event('Event Loop Blocked',
					`Event loop blocked for ${ms}ms on C${this.dyno.clientOptions.clusterId}`,
					{ alert_type: 'warning' },
					[this.config.stateName, `C${this.dyno.clientOptions.clusterId}`]);
			}
