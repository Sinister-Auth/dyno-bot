...
class Dyno extends Module {
	...
	async start() {
		...
		this._blocked = blocked(ms => {
			if (ms > 10000) {
				this.statsd.event('Event Loop Blocked',
					`Event loop blocked for ${ms}ms on C${this.dyno.options.clusterId}`,
					{ alert_type: 'warning' },
					[this.config.stateName, `C${this.dyno.options.clusterId}`]);
			}
			this.statsd.increment(`node.blocked.${this.config.stateName}`, 1);
		}, { threshold: 100 });
	...
	onGuildCreate(guild) {
		...
		this.statsd.increment('guild.events.create', 1);
	}
	...
	onGuildDelete(guild) {
		...
		this.statsd.increment('guild.events.delete', 1);
	}
	...
	onMessage() {
		this.statsd.increment('message.create', 1);
	}
	...
	onCommandSuccess({ command, time }) {
		...
		this.statsd.timing(`commands.${command.group || command.module}.${command.name}.time`, time);
	}
	...
	async updateStats() {
		...
		if (this.config.isCore) {
			try {
				await Promise.all([
					this.redis.hsetAsync(`dyno:guilds:${this.config.client.id}`, `${state}:${clusterId}`, this.client.guilds.size),
					this.redis.hsetAsync(`dyno:cstats:${this.config.client.id}`, `${state}:${clusterId}`, JSON.stringify(coredata)),
					]);
			} catch (err) {
				this.logger.error(err, { type: 'dyno.updateStats.coreStats' });
			}
			this.statsd.gauge(`guilds.${state}.${clusterId}`, data.guilds);
			this.statsd.gauge(`users.${state}.${clusterId}`, data.users);
			this.statsd.gauge(`gateway.${state}.${clusterId}`, this._events);
			this._events = 0;
		}

		try {
			await Promise.all([
				this.redis.hsetAsync(`dyno:vc`, `${state}:${clusterId}`, data.voice || 0),
				this.redis.hsetAsync(`dyno:stats:${state}`, clusterId || 0, JSON.stringify(data)),
				]);
		} catch (err) {
		...
		try {
			var [shards, vcs, ffmpegs] = Promise.all([
				this.redis.hgetallAsync(`dyno:stats:${this.config.state}`),
				this.redis.hgetallAsync(`dyno:vc`),
				this.getFFmpegs(),
			]);
		} catch (err) {};
