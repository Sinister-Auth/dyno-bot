...
export default class CommandModal extends React.Component {
    ...
    render() {
		...
		const allowedChannels = channels.filter(c => this.state.formData.allowedChannels.find(i => i.id === c.id));
		const ignoredChannels = channels.filter(c => this.state.formData.ignoredChannels.find(i => i.id === c.id));
