...
class Automod extends Module {
	...
	clearTimeouts() {
		if (this.rateLimits.size) {
			each([...this.rateLimits.keys()], key => {
				let limit = this.rateLimits.get(key);
				if (Date.now() - limit.createdAt < 4000) return;
				this.rateLimits.delete(key);
			});
		}

		if (this.moderator.warnings.size) {
			each([...this.moderator.warnings.keys()], key => {
				let warning = this.moderator.warnings.get(key);
		...
		if (this.linkCooldowns.size) {
			each([...this.linkCooldowns.keys()], key => {
				let cooldown = this.linkCooldowns.get(key);
		...
		if (this.attachCooldowns.size) {
			each([...this.attachCooldowns.keys()], key => {
				let cooldown = this.attachCooldowns.get(key);
				this.dyno.guilds.getOrFetch(cooldown.guild.id)
					.then(config => {
						if (!config) {
	...
	clearInfractions() {
		if (this.infractions.size) {
			each([...this.infractions.keys()], key => {
				let infraction = this.infractions.get(key);
	...
	messageCreate(e) {
		...
		for (let rule of rules) {
			if (!rule.condition || !this.filter[rule.filter]) continue;
