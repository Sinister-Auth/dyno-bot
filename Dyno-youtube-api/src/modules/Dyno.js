...
class Dyno extends Module {
	...
	async start() {
		...
		this._blocked = blocked(ms => {
			if (ms > 10000) {
				statsd.event('Event Loop Blocked',
	...
	async updateStats() {
		...
		if (this.config.isCore) {
			try {
				await Promise.all([
					redis.hsetAsync(`dyno:guilds:${this.config.client.id}`, `${state}:${clusterId}`, this.client.guilds.size),
					redis.hsetAsync(`dyno:cstats:${this.config.client.id}`, `${state}:${clusterId}`, JSON.stringify(coredata)),
					]);
			} catch (err) {
