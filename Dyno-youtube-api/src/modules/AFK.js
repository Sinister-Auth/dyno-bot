...
class AFK extends Module {
	...
	async setAFK(msg, status) {
		if (!await this.isEnabled(msg.channel.guild, this)) return Promise.reject();
		...
		let name = msg.member.nick || msg.author.username;
		...
		await msg.member.edit({ nick: `[AFK] ${name}` }).catch(() => null);
