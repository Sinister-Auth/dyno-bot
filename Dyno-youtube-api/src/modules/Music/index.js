...
class Music extends Module {
	...
	updateStats() {
		let players = this.client.voiceConnections.size,
			playing = [...this.client.voiceConnections.values()].filter(c => c.playing),
			clusterId = this.dyno.options.clusterId.toString(),
			state = this.config.state;

		statsd.gauge(`voice.total.${state}.${clusterId}`, players || 0);
		statsd.gauge(`voice.playing.${state}.${clusterId}`, playing || 0);
		redis.hsetAsync(`dyno:vc:${this.config.client.id}`, `${this.dyno.options.clusterId}:${this.config.state}`, playing.length || 0)
			.catch(() => null);
	}
	 ...
	 */
	checkConnections() {
		const players = this.client.voiceConnections;
		if (!players || !players.size) return;

		each([...players.values()], player => {
			let channel;
			...
			try {
				channel = this.client.getChannel(player.channelID);
			} catch (err) {
	...
	async getInfo(url, guildId, fetchAll) {
		...
		try {
			const ytApi = `${process.env.YT_API_HOST}:${process.env.YT_API_PORT}`;
			info = await superagent
									.get(`${ytApi}/info`)
									.query({ url: encodeURI(url) })
									.set('Authorization', process.env.YT_API_TOKEN);
			info = info.body;
			
		} catch (err) {
			//We supress the original error because ECONNREFUSED could give away the server IP
			return Promise.reject(new Error('An error occured when fetching the video information. Please try again later.'));
		}
	...
	getPlayingState(guildOrChannel) {
		...
		const player = this.client.voiceConnections.get(guildId);
	...
	async add(guildId, voiceChannel, url) {
		...
		if (!guildConfig.isPremium) {
			if (mediaInfo && mediaInfo.length > (this.maxSongLength || 5400)) {
				return Promise.reject('60m');
			}
			if (mediaInfo && mediaInfo.length < 30) {
				return Promise.reject('That song is less than 30 seconds long, please try another.');
			}
		}
