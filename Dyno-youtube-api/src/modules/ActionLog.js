...
class ActionLog extends Module {
	 ...
	 * @returns {*}
	 */
	onCommand({ command, message, guildConfig }) {
		if (!guildConfig) return;
		this.shouldLog({ event: 'commands', guild: message.channel.guild, channel: message.channel, guildConfig }).then(async (logChannel) => {
			if (!logChannel) return;
