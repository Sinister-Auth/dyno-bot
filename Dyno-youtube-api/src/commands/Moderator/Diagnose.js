...
class Diagnose extends Command {
	constructor(...args) {
		...
		this.aliases      = ['diagnose'];
		this.group        = 'Moderator';
		this.description  = 'Diagnose any command or module in the bot to determine if there are any problems.';
		this.usage        = 'diagnose [command or module]';
	...
	async execute(e) {
		...
		if (message.guild.id === this.config.dynoGuild && args.length > 1 && args[args.length - 1].match(/([0-9]+)/)) {
			if (this.isServerMod(message.member, message.guild.defaultChannel)) {
				let guildConfig = await this.dyno.guilds.getOrFetch(args.pop());
				if (!guildConfig) {
		...
		const command = this.dyno.commands.find(c => c.name === args[0] && c.permissions !== 'admin');

		if (command) {
			return this.diagnoseCommand(command, e);
		}

		const module = this.dyno.modules.find(m => !m.core &&
	...
	diagnose({ message, guildConfig, perms }) {
	...
	diagnoseCommand(command, { message, guildConfig }) {
		...
		if (command.permissions === 'serverAdmin') {
			diagnosis.info.push(`The command requires admin permissions.`);
		}

		if (command.permissions === 'serverMod') {
			diagnosis.info.push(`The command requires mod permissions.`);
		}
	...
	diagnoseModule(module, { message, guildConfig, remote }) {
		const globalConfig = this.dyno.globalConfig;
